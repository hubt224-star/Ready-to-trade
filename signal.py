...
st.title("🎯 Institutional Options & Commodity Signal Engine")
...
# Updated Dictionary with Equity Indices and Commodities
ASSETS = {
    "NIFTY 50": "^NSEI",
    "SENSEX": "^BSESN",
    "BANK NIFTY": "^NSEBANK",
...
def calculate_signal_score(df):
    ...
    if bull_score >= min_score:
        signal = f"STRONG BULLISH BUY 🟢 [{bull_score}% Match]"
        trade_type = "CALL"
        sl = round(price - (1.5 * atr), 2)
        target = round(price + (3.0 * atr), 2)
    elif bear_score >= min_score:
        signal = f"STRONG BEARISH SELL 🔴 [{bear_score}% Match]"
        trade_type = "PUT"
        sl = round(price + (1.5 * atr), 2)
        target = round(price - (3.0 * atr), 2)
...
def display_asset_card(name, symbol):
    ...
    if df is not None and not df.empty:
        ...
        m1.metric("Live Price", f"${price}" if "=F" in symbol else f"₹{price}")
        ...
        if sl > 0 and target > 0:
            ...
            currency = "$" if "=F" in symbol else "₹"
            rc1.metric("Suggested Stop-Loss", f"{currency}{sl}")
            rc2.metric("Target (1:2 R:R)", f"{currency}{target}")
...
# Main Display Tabs
tab1, tab2 = st.tabs(["📈 Equity Indices", "🛢️ Commodities"])

with tab1:
    col1, col2 = st.columns(2)
    with col1:
        display_asset_card("NIFTY 50", ASSETS["NIFTY 50"])
    with col2:
        display_asset_card("SENSEX", ASSETS["SENSEX"])

with tab2:
    col_c1, col_c2 = st.columns(2)
    with col_c1:
        display_asset_card("Gold", ASSETS["Gold (Futures)"])
        display_asset_card("Crude Oil", ASSETS["Crude Oil (Futures)"])
        display_asset_card("Copper", ASSETS["Copper (Futures)"])
    with col_c2:
        display_asset_card("Silver", ASSETS["Silver (Futures)"])
        display_asset_card("Natural Gas", ASSETS["Natural Gas (Futures)"])
